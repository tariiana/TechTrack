...
async function getAll(req, res, next) {
  try {
    ...
    res.json(ResponseFormatter.success(instruments));
  } catch (err) { next(err); }
}
...
async function getById(req, res, next) {
  try {
    ...
    res.json(ResponseFormatter.success(instrument));
  } catch (err) { next(err); }
}
...
async function create(req, res, next) {
  try {
    ...
    res.status(201).json(ResponseFormatter.created(instrument, 'СИ создано'));
  } catch (err) {
    if (err.code === '23505') {
...
async function update(req, res, next) {
  try {
    ...
    res.json(ResponseFormatter.success(updated, 'СИ обновлено'));
  } catch (err) {
    if (err.code === '23505') {
      return res.status(409).json(ResponseFormatter.error('Табельный номер уже существует', 409));
    }
...
async function writeOff(req, res, next) {
  try {
    ...
    if (!instrument) return res.status(404).json(ResponseFormatter.error('СИ не найдено', 404));
    res.json(ResponseFormatter.noContent('СИ списано'));
  } catch (err) { next(err); }
}
...
async function getVerifications(req, res, next) {
  try {
    ...
    res.json(ResponseFormatter.success(verifications));
  } catch (err) { next(err); }
}
...
async function addVerification(req, res, next) {
  try {
    ...
    res.status(201).json(ResponseFormatter.created(verification, 'Поверка добавлена'));
  } catch (err) { next(err); }
}
module.exports = {
  getAll,
